...
- **Stages 0–10 + backlog PRs A–P shipped.** Full backend (requisitions, candidates, interviews + state machine, headcount, spreadsheet import, document uploads, dashboard aggregations, **applications inbox**, **multi-TA assignment**, **AOP-edit UX**, **admin change notifications**, **per-user city-access scoping**), CI/CD on push to `main`, and OpenAPI docs at `/api/docs`.
