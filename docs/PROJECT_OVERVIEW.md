...
- **Stages 0–10 + backlog PRs A–Q + post-launch hardening shipped.** Full backend (requisitions, candidates, interviews + state machine, headcount, spreadsheet import, document uploads, dashboard aggregations, **applications inbox**, **multi-TA assignment**, **AOP-edit UX**, **admin change notifications**, **per-user city-access scoping**, **BU-based auto-routing of req approval**), CI/CD on push to `main`, and OpenAPI docs at `/api/docs`. As of May 12 2026 the entire Apr 29 beta-feedback backlog and all 36 findings from the May code review are closed.
...
- `src/App.jsx` — main app, ~3000 lines, sidebar layout
